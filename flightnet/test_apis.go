...
// Since we're in package main, we need to define the structs and functions locally
// or import them. For this test, we'll define what we need locally.
...
func main() {
	...
	// Test file structure (looking in clients directory)
	...
	fmt.Println("   1. Add API keys to clients/.env file")
	fmt.Println("   2. Run from clients directory: cd clients && go run test_apis.go")
	fmt.Println("   3. Or move test to clients directory for proper package access")
}
...
func testBasicFunctionality() {
	...
	// Test environment variables from clients/.env
	fmt.Println("  📋 Testing environment setup...")

	// Check if .env file exists in clients directory
	clientsEnvPath := filepath.Join("clients", ".env")
	if envContent, err := os.ReadFile(clientsEnvPath); err == nil {
		envStr := string(envContent)
		fmt.Printf("  ✅ Found .env file in clients directory (%d bytes)\n", len(envContent))

		if containsPattern(envStr, "AVIATION_EDGE_API_KEY=") {
			fmt.Printf("  ✅ AVIATION_EDGE_API_KEY configured in clients/.env\n")
		} else {
			fmt.Printf("  ⚠️  AVIATION_EDGE_API_KEY not configured in clients/.env\n")
		}

		if containsPattern(envStr, "ICAO_API_KEY=") {
			fmt.Printf("  ✅ ICAO_API_KEY configured in clients/.env\n")
		} else {
			fmt.Printf("  ⚠️  ICAO_API_KEY not configured in clients/.env\n")
		}
	} else {
		fmt.Printf("  ❌ Cannot read clients/.env file: %v\n", err)
	}
...
func testFileStructure() {
	...
	fmt.Println("  📋 Checking required files in clients directory...")
	...
	for _, file := range requiredFiles {
		filePath := filepath.Join(clientsDir, file)
	...
	// Check .env file content in clients directory
	fmt.Println("  📋 Checking clients/.env file...")
	envPath := filepath.Join(clientsDir, ".env")
	if envContent, err := os.ReadFile(envPath); err == nil {
		envStr := string(envContent)
		if len(envStr) > 0 {
			fmt.Printf("  ✅ clients/.env file has content (%d bytes)\n", len(envContent))
			...
			fmt.Printf("  ⚠️  clients/.env file is empty\n")
			fmt.Printf("  📝 Add your API keys to clients/.env like:\n")
	...
	// Check Go module in clients directory
	fmt.Println("  📋 Checking clients/go.mod...")
	modPath := filepath.Join(clientsDir, "go.mod")
	if _, err := os.Stat(modPath); err == nil {
		fmt.Printf("  ✅ clients/go.mod exists\n")
		if modContent, err := os.ReadFile(modPath); err == nil {
			fmt.Printf("  📝 Module content: %s\n", string(modContent))
		}
	} else {
		fmt.Printf("  ⚠️  clients/go.mod missing\n")
	}
}
